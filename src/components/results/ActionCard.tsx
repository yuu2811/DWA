...
export default function ActionCard({ action, index, completed, onToggle }: ActionCardProps) {
  ...
  return (
    <div
      ...
      role="article"
      aria-label={`${cfg.label}: ${action.title}`}
      style={{
    ...
    >
      ...
      <div className="w-full px-4 py-3.5 flex items-start gap-3 text-left">
        ...
        <button
          ...
          aria-label={completed ? `${action.title} を未完了にする` : `${action.title} を完了にする`}
